...
#     c_4 = c_4_ind
    ...
    #loss = tf.Print(loss, [loss4], message="loss4")
    ...
    loss = loss + tf.clip_by_value((2 - lod_in), 0.0, 1.0)*(loss3 + loss4 +loss5)
    
    if D.output_shapes[1][1] > 0:
        with tf.name_scope('LabelPenalty'):
            label_penalty_fakes = tf.nn.softmax_cross_entropy_with_logits_v2(labels=labels, logits=fake_labels_out)
        loss += label_penalty_fakes * cond_weight
    tfutil.autosummary('Loss/GFinalLoss', loss)
    return loss
...
def D_wgangp_acgan(G, D, opt, training_set, minibatch_size, reals, labels,
    ...
    tfutil.autosummary('Loss/DFinalLoss', loss)
    return loss
